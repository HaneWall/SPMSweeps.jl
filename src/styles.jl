...
function plot_sweep_control_phase(sweeps::Array{S}, controls=Array{S}; k=1) where {S<:Nanojunction}
  ...
  for p in sweeps
    ...
    omega_drivings_fwd .= p.matrix_harmonic_state_converged_sweep[1, :]
    phaselag_1 .= atan.(p.matrix_harmonic_state_converged_sweep[2*k+2, :], p.matrix_harmonic_state_converged_sweep[2*k+1, :])
    lines!(ax_a, omega_drivings_fwd[1:end], phaselag_1[1:end], linewidth=MARKERSIZES[n_counter])
    n_counter += 1
  end
  ...
  for p in controls
    ...
    scatter!(ax_a, omega_drivings_fwd[1:end], phaselag_1[1:end], marker=MARKERS[n_counter], markersize=MARKERSIZES[n_counter], color=COLORS[3])
  end
  ylims!(ax_a, (-π, 0))
  xlims!(ax_a, (0.989, 1.011))
  return fig
end


function plot_error(p::Nanojunction, idx_min::Integer, idx_max::Integer; Δt=0.05)
  CairoMakie.activate!(type="svg", pt_per_unit=1)
  fig = Figure(
    fontsize=7,
    size=(230, 85),
  ...
  ax_err = Axis(fig[1, 1],
    ...
    xgridvisible=false,
    ygridvisible=false)
  time_ms = collect(1:(idx_max-idx_min+1)) ./ (1 / Δt) .* 1 / (p.ω_0 / (2π)) .* 1000
  lines!(ax_err, time_ms, p.error[idx_min:idx_max] .* 1000, linewidth=2)
  ylims!(ax_err, -51, 40)
  return fig
end

#nature paper index Lennard Jones 534000, 597000
function plot_ctrl(p::Nanojunction, idx_min::Integer, idx_max::Integer; Δt=0.05)
  CairoMakie.activate!(type="svg", pt_per_unit=1)
  fig = Figure(
    fontsize=7,
    size=(232, 80),
  ...
  ax_err = Axis(fig[1, 1],
    ...
    ygridvisible=false)
  time_ms = collect(1:(idx_max-idx_min+1)) ./ (1 / Δt) .* 1 / (p.ω_0 / (2π)) .* 1000
  lines!(ax_err, time_ms, p.current_ctrl[idx_min:idx_max], linewidth=2)
  ylims!(ax_err, 0.979, 1.021)
  return fig
end
...
function plot_sweep_control_amplitude(sweeps::Array{S}, controls=Array{S}; k=1) where {S<:Nanojunction}
  ...
  for p in sweeps
    ...
    lines!(ax_a, omega_drivings_fwd[1:end], amplitude_1[1:end], linewidth=MARKERSIZES[n_counter])
    n_counter += 1
  end
  ...
  for p in controls
    ...
    scatter!(ax_a, omega_drivings_fwd[1:end], amplitude_1[1:end], marker=MARKERS[n_counter], markersize=MARKERSIZES[n_counter], color=COLORS[3])
  end
  ylims!(ax_a, (0.0, 65))
  xlims!(ax_a, (0.989, 1.011))
  return fig
